...
![image-20210720194008781](https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/Elasticsearch/image-20210720194008781.png)
...
![image-20210904173404121](https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/分布式搜索引擎01/image-20210904173404121.png)
...
![image-20210721215729236](https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/分布式搜索引擎01/image-20210721215729236.png)
...
**[聚合（](https://www.elastic.co/guide/en/https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/Elasticsearch/reference/current/search-aggregations.html)[aggregations](https://www.elastic.co/guide/en/https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/Elasticsearch/reference/current/search-aggregations.html)[）](https://www.elastic.co/guide/en/https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/Elasticsearch/reference/current/search-aggregations.html)**可以让我们极其方便的实现对数据的统计、分析、运算。例如：
...
声明自定义分词器的语法如下：
**注意，自定义分词器只针对当前选中的索引库！！！，因从我们要想使用自定义分词器只能在相应的索引库中使用**

下面首先将词语使用 `ik_max_word`进行中文分词，紧接着将分好的词使用自定义的 `py`filter过滤器进行过滤，过滤器实际上呢使用可拼音分词器，最后就会多出来非常多的term！！！

但是存在致命问题：加入文档中有狮子、柿子两个文档，我搜索时使用 `狮子`关键词来进行搜索，这就会导致两个文档全通过拼音搜索到，因此我们实际在搜索的是时候不允许使用pinyin分词，即搜索不使用自定义分词：

![image-20241128221041627](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/2024-11-aabdbc5af1ce2a50ce6e8a24fb996cbe.png)
...
elasticsearch提供了[Completion Suggester](https://www.elastic.co/guide/en/https://raw.githubusercontent.com/limingzhong61/LearningNotes/5f57182e77161f80d8cbef343acc3756d5f0114d/Java/SpringCloud/Elasticsearch/Elasticsearch/reference/7.6/search-suggesters.html)查询来实现自动补全功能。这个查询会匹配以用户输入内容开头的词条并返回。为了提高补全查询的效率，对于文档中字段的类型有一些约束：

- 参与补全查询的字段必须是completion类型：这个是硬性要求，必须的，因为补全实际上是将用户的输入补全为某个字段

- 字段的内容一般是用来补全的多个词条形成的数组：这是因为补全实际上是将用户的输入补全为某个字段，加入有以下例子：![image-20241128221406852](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/2024-11-ffde9b36798415c9ebf27bb5f0b55416.png)

    上图红色框中的 我输入 `S`会进行补全，输入`WH`也会进行补全，这样就很牛逼了，因此尽量把每个字段使用数组分成多个词条，这样即使从中间某个部分开始输入也能够搜索的到

![image-20241128221706743](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/2024-11-fcf17fccb6ecdec8df42c61a28e5c4af.png)
...
![image-20241128222812053](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/2024-11-79849736c36efd5094794c4eb0c1cabb.png)

在这里，当我输入`shi`，实际上调用的是自动补全功能，当我点击搜索，实际上调用的是 `全局搜索功能`，两者提供了不同的服务；
...
注意：**topic交换机，的特点是允许路由键模糊匹配选择发送的队列，*匹配单个字符，#匹配0或多个字符**

还有：我们在SpringBoot使用RabbitMq，`Spring AMQP`会自动为我们提供一定的消息转换功能，特别是 `convertAndSend`方法，会将发送的消息转化成 String、Byte【】等，使用MessageConvert来进行自动转换，

```java
@RabbitListener(queues = MqConstants.HOTEL_INSERT_QUEUE)
public void handleHotelInsert(Hotel hotel) {
    // 处理接收到的酒店消息
}
```

在上述接收示例中，当接收到 `HOTEL_INSERT_QUEUE` 队列中的消息时，Spring AMQP 会尝试将消息转换为 `Hotel` 对象。转换会基于消息的格式自动进行，如果消息是 JSON 格式的，Spring AMQP 会使用默认的 `Jackson2JsonMessageConverter`（如果配置了）来进行 JSON 到 Java 对象的转换。
...
public class MqConfig {
        // 定义一个名为 topicExchange 的 Bean，返回一个 TopicExchange 类型的实例
    @Bean
    public TopicExchange topicExchange(){
        // 创建一个主题交换机，名字从 MqConstants.HOTEL_EXCHANGE 获取
        // 第一个参数是交换机的名字，第二个参数表示交换机是否持久化，第三个参数表示是否自动删除
        return new TopicExchange(MqConstants.HOTEL_EXCHANGE, true, false);
    }

    // 定义一个名为 insertQueue 的 Bean，返回一个 Queue 类型的实例
    @Bean
    public Queue insertQueue(){
        // 创建一个名为 HOTEL_INSERT_QUEUE 的消息队列，队列是持久化的
        return new Queue(MqConstants.HOTEL_INSERT_QUEUE, true);
    }

    // 定义一个名为 deleteQueue 的 Bean，返回一个 Queue 类型的实例
    @Bean
    public Queue deleteQueue(){
        // 创建一个名为 HOTEL_DELETE_QUEUE 的消息队列，队列是持久化的
        return new Queue(MqConstants.HOTEL_DELETE_QUEUE, true);
    }

    // 定义一个名为 insertQueueBinding 的 Bean，返回一个 Binding 类型的实例
    @Bean
    public Binding insertQueueBinding(){
        // 使用 BindingBuilder 将 insertQueue 和 topicExchange 绑定起来，并指定路由键（routing key）
        // 路由键是 MqConstants.HOTEL_INSERT_KEY，表示该队列将接收与该路由键匹配的消息
        return BindingBuilder.bind(insertQueue()).to(topicExchange()).with(MqConstants.HOTEL_INSERT_KEY);
    }

    // 定义一个名为 deleteQueueBinding 的 Bean，返回一个 Binding 类型的实例
    @Bean
    public Binding deleteQueueBinding(){
        // 使用 BindingBuilder 将 deleteQueue 和 topicExchange 绑定起来，并指定路由键（routing key）
        // 路由键是 MqConstants.HOTEL_DELETE_KEY，表示该队列将接收与该路由键匹配的消息
...
- **海量数据**存储问题：将索引库从逻辑上拆分为N个分片（shard），存储到多个节点
- **单点故障**问题：将分片数据在不同节点备份（replica ）
...
搭建步骤见：`xblog`github仓库，关于搭建ElasticSearch过成证书配置问题：

1. 证书是有必要配置的，因为若不配置那么es服务端各个节点之间的通信以及服务端与客户端之间的通信都将使用明文进行传输，非常不安全

2. 虽然可以使用一份证书（例如一个公共的自签名证书）来配置集群中的所有节点，但更常见的做法是每个节点拥有 **唯一的证书**，以确保每个节点之间的身份验证和加密通信，因为在实际部署不很可能每个es节点不在同一个主机上， 他们拥有不同的IP地址
3. 对于客户端来说，**客户端需要信任 Elasticsearch 节点的证书**，才能建立安全的 TLS/SSL 连接。如果客户端不信任该证书（无论是自签名证书还是由第三方 CA 签发的证书），则它将无法与 Elasticsearch 集群建立通信。

>  curl -v https://your-es-cluster:9200

使用此命令可以判断集群有没有使用https通信；
...
分片是es将数据分配到集群中不同节点上的基本单位，我们在创建索引的同时可以指定**分片数量**和**副本数量**，
...
分片数量对性能有一定影响，具体影响包括以下几个方面：

+ 由于每一个分片都是一个独立的lucene实例，过的分片导致cpu、内存资源的浪费，并且节点间的通信负担增加，但是过少的分片导致每个分片上的数据过多，导致查询效率大大减低，

即使是单节点的es，也是会有分片（shards）和副本（replica）概念，默认的分片数量为5；
