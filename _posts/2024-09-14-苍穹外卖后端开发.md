...
我们在数据库中有很多的公共字段例如：创建时间、创建人、修改时间、修改人等，我们想早做的就是在我们修改、插入某条数据时自动为我们自动填写这些公共字段，而不是通过我们在service层做这些填充；

分析：公共字段的修改主要集中在 插入、修改操作上，主要集中在持久层的某些插入修改方法上，即mapper接口的某些方法

方法呢就是首先自定义注解用于标记那些方法需要被拦截，不增加注解的不用处理，之后通过APO面向切面编程来进行拦截带有注解的方法，在通过注解为公共字段进行赋值：![image-20240919143713672](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240919143713672.png)

#### 定义注解

##### 举例

自定义注解可以结合反射和 AOP 实现非常强大的功能。
...
@Retention(RetentionPolicy.RUNTIME)  // 指定注解在运行时可见（保证使用反射可以获取到）
public @interface MyCustomAnnotation {
    String value() default "default value";  // 定义一个属性，默认为 "default value"
    int age() default 20; // 定义一个int属性，默认为20
}
```

@target注解用于指定注解可以使用的地方，一般有：
...
通过反射，我们可以先通过获取方法类，再通过方法类得到注解，并得到注解中的各种属性；
...
切面直观理解就是用来将横切点（如日志、权限）与业务逻辑分离的模块。

##### 切入点：

注解：**@Pointcut(execution="")**

切入点定义了在哪些地方插入切面逻辑。常见的切入点表达式使用 `execution` 表达式：

`execution(* com.example.service.*.*(..))`：表示拦截 `com.example.service` **包**下的所有方法。

- `*`：匹配任意返回类型。
- `com.example.service.*`：匹配 `com.example.service` 包下的所有类。
- `.*(..)`：匹配所有方法，任意参数。

当然execution表达式不仅仅能匹配这些，`execution` 表达式可以使用逻辑运算符 `&&`、`||` 和 `!` 进行多个切入点的组合，并且可以捕获带有特定注解的方法。

例如： `execution(* com.example.service.*.*(..)) && @annotation(com.example.annotation.MyCustomAnnotation)`用于匹配 com.example.sevice包下的所有带有特定注解的方法；

##### 通知类型Advice

Spring AOP 支持多种通知类型，可以在方法执行的不同阶段执行切面逻辑。

- `@Before`：在目标方法执行之前运行。
- `@After`：在目标方法执行之后运行。
- `@Around`：在目标方法执行前后都运行，最灵活的通知类型。
- `@AfterReturning`：在目标方法成功返回结果之后运行。
- `@AfterThrowing`：在目标方法抛出异常时运行。

通知函数通常会有一个 `JointPoint`类型参数，表示当前执行的连接点（即被拦截的方法），可以通过此参数获取到被拦截方法的详细信息，主要`API`如下：

**`joinPoint.getSignature()`**：获取方法的签名，包括方法名和参数类型。

**`joinPoint.getArgs()`**：获取被拦截方法的实际参数值。

**`joinPoint.getTarget()`**：获取执行该方法的目标对象（即业务类对象）。
...
public class AutoFillAspect {
    /**
     * 切入点表达式，拦截所有mapper层的带有AutoFill注解的方法
     */
    ...
    public void autoFill(JoinPoint joinPoint) {
        ...
        // 获取注解指定方法的参数,即数据库操作类型
        MethodSignature signature = (MethodSignature) joinPoint.getSignature();
        AutoFill annotation = signature.getMethod().getAnnotation(AutoFill.class);
        OperationType operationType = annotation.value();
        // 获取被拦截方法参数,这里我们自行约定，第一个参数为实体类对象！！！
        ...
        if(operationType == OperationType.UPDATE){
            ...
            try {
                ...
                setUpdateTime.invoke(args[0], localDateTime);
                setUpdateUser.invoke(args[0], id);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }else if(operationType == OperationType.INSERT){
            // 为四个字段赋值，下面的常量方法名有多次使用，我们将其分装成一个常量
            try {
                Method setUpdateTime = args[0].getClass().getMethod(AutoFillConstant.SET_UPDATE_TIME, LocalDateTime.class);
                Method setUpdateUser = args[0].getClass().getMethod(AutoFillConstant.SET_UPDATE_USER, Long.class);
                ...
                setUpdateTime.invoke(args[0], localDateTime);
                setUpdateUser.invoke(args[0], id);
                setCreateTime.invoke(args[0], localDateTime);
...
其中有一个非常重要的属性： `enctype`，这个了属性指定了向服务器发送表单之前如何对其进行编码，适用于 `POST`请求；

只有当enctype指定了 "multipart/form-data"，springboot后端才可以正确接收到文件内容以及其他参数；

springboot端：

一样，除了文件以外的其他参数我们都可以直接接收的，文件我们得使用一个单独的 `MultipartFile`对象来接收(参数名对应即可)：

![image-20240920094237730](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240920094237730.png)
...
public class UploadController{
    ...
    public Result upload(String username,Integer age, MultipartFile image){
        ...
        String newFilename = uuid + extName;
       	// 保存文件
        img.transforTo(new File("E://images/" + newFilename));
        return Result.success();
    }
}
```

**注意**：Springboot中文件上传默认最大为1M，如果需要上传大文件我们需要再配置文件中这样设置：

![image-20240920093853045](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240920093853045.png) 

以上代码将上传文件设置在了服务器本地磁盘，存在几点问题：

1. 存储后的图片等前端页面无法直接访问
2. 后端磁盘容量有限，可能不方便扩容
3. 服务器磁盘坏了，则很少推荐

在实际开发中我们最常用的还是阿里云的**云服务**：![image-20240920094426891](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240920094426891.png)

注册一个阿里云oss服务，得到id和secretid，配置以下即可，下面是工具类：
```java
public class AliOssUtil {
    /*com.aliyun.oss 是阿里云对象存储服务（OSS，Object Storage Service）的官方 Java SDK 包，
    用于在 Java 应用中访问和管理阿里云 OSS 服务。它提供了一系列的 API，可以让开发者方便地对 OSS 进行
    文件上传、下载、删除、列举、管理 Bucket 等操作。*/
...
数据库中设置主键自增长，很简单，使用`auto_increase`即可，但是，如果我们想在插入后获取到这个自增长id就会有一些麻烦，但是mybatis好像为我们设置好了这个机制，使用 `useGeneratedKeys`和 `keyProperty`以及 `keyColumn`关键字即可，能够做到将自增长的id**直接赋值给传进来的对象属性**！！！。对于支持自动生成记录主键的数据库，如：MySQL，SQL Server，此时设置`useGeneratedKeys`参数值为true，在执行添加记录之后可以获取到数据库自动生成的主键ID。
...
在 `SQL` 的 `LEFT JOIN` 中，对于左表的每一项，右表可能出现三种情况：

**没有匹配项**：返回左表的行，右表字段为 `NULL`。

**有一项匹配项**：返回左表与右表的匹配行。

**有多项匹配项**：左表行与右表的多行匹配，结果会返回多条记录。

#### 事务管理使用

当Service服务涉及的数据库超过2个时，我们需要

#### Redis简单使用

基本数据额类型介绍：

![image-20240923151735774](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923151735774.png)

数据操作：

（1）字符串操作：

![image-20240923151820331](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923151820331.png)

我们可以看到，**设置过期时间**，比较常用的应用场景是 “验证码”；

（2）Hash表操作

![image-20240923181330612](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923181330612.png)

（3）list操作（队列）

![image-20240923181449774](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923181449774.png)

例如：lrange 1 -1
（3）集合

![image-20240923181745237](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923181745237.png)

（4）有序集合

![image-20240923182353861](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923182353861.png)

（5）通用命令

主要是操作key的。

![image-20240923182727680](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923182727680.png)

java操作Redis：

![image-20240923183102799](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923183102799.png)
...
![image-20240923201846998](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923201846998.png)

![image-20240923213001067](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240923213001067.png)



由于营业状态就一个 `status`我们根本没必要专门给它设置一个数据库，因此直接使用redis；

Redis数据库存储的时候，key的存储其实也是可以成树形结构展示的，但这对key的命名有要求，需要使用 `:`作为分隔符；效果如图：

![image-20240926210943307](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240926210943307.png)
...
public class HttpClientUtil {
    /*httpclient该库主要用于处理 HTTP 协议，提供了强大的 HTTP 客户端功能。*/
    ...
    /**
     * 发送POST方式请求，请求体中的格式为键值对格式，模拟网页表单提交：name=John&email=john%40example.com
     * @param url
     * @param paramMap
     * @return
     * @throws IOException
...
对于用户端来说，存在大量并发的情况，此时就不适宜并发对数据库进行访问，其会成为性能瓶颈。

实现思路如下：

![image-20240926134456996](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240926134456996.png)

对于后端需要访问数据库的服务（service）：

1. 首先判断是否存在缓存，如有，直接读取缓存
2. 没有，则访问数据库获取数据，并将读取的数据载入缓存
3. （对于数据库更新、插入操作后，要及时更新缓存

对于此外卖项目来说，我们使用redis作为缓存数据库，其中分类IDCategoryId作为key；

注意：新增、修改、删除、状态修改等操作都需要 删除Redis缓存！！！！！！



springboot已经为我设计好了相关的注解方法，让我们非常方便的管理Redis缓存；（进一步减少缓存代码的开发）

spring cache是一个框架，是提供了一层抽象，其底层可以使用不同的缓存实现，例如：
...
但我们导入Redis客户端的maven的坐标依赖时，Cache框架就会自动识别并使用其作为底层缓存实现。

常用注解：

| 注解名        | 说明                                                         |
| ------------- | ------------------------------------------------------------ |
| @EnableCachin | 开启缓存注解功能，通常加到启动类上                           |
| @Cacheable    | 在方法执行前先查询缓存中是否存在数据，有则直接取数据，没有则调用方法并将**方法返回值** 放置缓存中（既可以取，也可以放）(GET方法) |
| @CachePut     | 将**方法的返回值**放置到缓存中（只能放）（POST方法、Put方法） |
| @CacheEvict   | 将一条或者多条数据从缓存中删除（删除缓存）（Delete方法）     |

@CachePut使用：

![image-20240926205239611](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240926205239611.png)

@Cacheable使用：

使用方式一样，就是key参数使用的时候需要注意，

关于SpEL表达式，在源码中有这样一段话：

SpEL 表达式针对提供以下元数据的专用上下文进行评估：
`#result` 以获取对方法调用结果的引用。对于支持的包装器（如 Optional）， `#result` 是指实际对象，而不是包装器
...
![image-20240927093556002](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240927093556002.png)

### 设置购物车

在设计数据库的时候，其实有时候可以设置一些冗余字段，这样我们可以让我更加快速的进行查询，举例如下：![image-20240927163629353](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240927163629353.png)

比如说商品价格、图片地址以及商品名称， **并不会经常变化**，对于这些不经常变化的字段，我们可以设置为冗余字段，这样就不用每次都查表了；
...
这里面最重要的就是 `cron`表达式，表达式非常复杂，但是有现成的网站，在线生成cron表达式；例如：

> https://cron.ciding.cc/

特殊注意的点：周和日这两个字段一般只写其中一个，另一个使用？来代替
...
使用需要导入 `spring-context`，但是这个 jar 包是自动导入的，不需要额外导入；

其次，启动类需要添加 `@EnableScheduling`标签，

在需要使用定时任务的标签上使用 `@Scheduled(cron = "0/5 * * * * ?")`注解



### webSocket

![image-20240928212751441](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240928212751441.png)

传统的http协议只能由客户端一方发起请求，服务器一方接受请求返回一个response，并且每次发送request都需要有一次建立连接的过程，从使用、到效率上来说都是有一定缺陷的。而WebSocket连接，只需要建立一次连接，连接建立之后双方都可以成为消息的发送者；
...
![image-20240928212929374](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240928212929374.png)

（3）体育实况更新（各种实况更新类的网页）![image-20240928213029681](https://raw.githubusercontent.com/mikeaaaaaa/cloudimg/main/img/image-20240928213029681.png)

（4）股票实时更新，由服务端主动推送数据到客户端
